...
async function enroll(courseId: number): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(data.message ?? 'No se pudo completar la matrícula.');
    }
...
<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <div>
        <h1 class="text-2xl font-bold tracking-tight">Matrícula online</h1>
        ...
        >
          <CardHeader>
            <div class="flex items-start justify-between">
              <div>
                <CardTitle>Paralelo {{ course.parallel }}</CardTitle>
                <CardDescription>{{ course.level.name }}</CardDescription>
              </div>
              <Badge :variant="isFull(course) ? 'destructive' : 'default'">
          ...
          <CardContent class="space-y-4">
            ...
            <div class="text-sm">
              ...
              <span class="text-foreground font-semibold">
                ${{ Number(course.level.enrollment_fee).toFixed(2) }}
              </span>
            </div>
            <Button
              class="w-full"
            ...
            >
              <Spinner
                ...
                data-icon="inline-start"
              />
              {{ enrolling ? 'Procesando...' : 'Seleccionar paralelo' }}
            </Button>
          </CardContent>
        </Card>
      </div>
